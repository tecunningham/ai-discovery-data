...
**Data:** [`../data/openssl-vulnerabilities.csv`](../data/openssl-vulnerabilities.csv); per-finder rows in [`../data/discovery-finders.csv`](../data/discovery-finders.csv)
...
![Annual OpenSSL vulnerability disclosures, split by explicit AI and fuzzer credit.](../figures/discovery-cyber-openssl.png)
...
The CSV is built by `build_openssl()` in
...
by" string against two regexes shared with the Firefox series: `DISCOVERY_AI`
...
"LLM" and "agent", and `DISCOVERY_FUZZ` matches "fuzz". AI is tested first, so a
...
credit lines in `discovery-finders.csv`, 18 name Aisle Research and 9 name
...
[all software: disclosed](cyber-nvd-disclosed.md). The two other fixed
codebases, [curl](cyber-curl.md) and [Firefox](cyber-firefox.md), are the
