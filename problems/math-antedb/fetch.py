...
Run: python3 problems/math-antedb/fetch.py --expdb path/to/expdb

Writes this folder's antedb-bounds.csv and antedb-sweep.csv, which figure.py
then plots. The extraction is separated from the plotting because it needs the
ANTEDB source tree and a cddlib-backed pycddlib, neither of which belongs in
this repo's dependencies — the CSVs are vendored the same way the OWID
66-technology CSV is, and this script is run by hand.
...
One year (1991) trips a TypeError inside the database's own beta solver. The
year is reported as a skip rather than hidden.
...
Setup (roughly two minutes, all outside this repo):
    ...
    python3 problems/math-antedb/fetch.py --expdb path/to/expdb
...
OUT = HERE / "antedb-bounds.csv"
OUT_SWEEP = HERE / "antedb-sweep.csv"
