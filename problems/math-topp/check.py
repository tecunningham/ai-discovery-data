...
def main() -> int:
    rows = read_csv(HERE / "topp-problems.csv")
    status = Counter(row["status"] for row in rows)
    resolved = sorted(
        (int(row["resolved_year"]), row["problem_id"]) for row in rows
        if row["status"] == "resolved" and row["resolved_year"])
    dated = [year for year, _ in resolved]
    ...
    if dated and dated[0] >= list_year:
        failures.append("no event predates the list_year any more; the "
                        "**earliest event:** fact line is stale")

    claims = {
        f"**rows:** {len(rows)} scored; {len(dated)} resolved with a dated "
        f"year; {status['open']} open; {status['partial']} partial":
            "rows fact",
        ...
        f"**earliest event:** {resolved[0][0]} (entry {resolved[0][1]}), "
        f"predating the project's {list_year} start": "earliest-event fact",
