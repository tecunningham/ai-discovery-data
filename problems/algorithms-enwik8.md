...
**Data:** [`../data/compression-records.csv`](../data/compression-records.csv), rows with series `hutter_enwik8`
...
![The four awarded enwik8 Hutter Prize records from 2006 to 2017, ending before the agent era.](../figures/discovery-algorithms-enwik8.png)
...
the live version is the [enwik9 series](algorithms-enwik9.md).
...
The same generator as its successor: `compression_chart("hutter_enwik8", ...)` in
[`../tools/make_figures.py`](../tools/make_figures.py) reads
`compression-records.csv`, keeps the rows whose `series` column is
...
the enwik9 series, because the Large Text Compression Benchmark rows in the CSV
are all for the 1 GB corpus. The legend still carries the collection's shared key,
...
counterfactual for the live series. On [enwik9](algorithms-enwik9.md) the awarded
...
step on [modded-nanogpt](algorithms-nanogpt.md) and about 23% once on the
[CIFAR-10 speedrun](algorithms-cifar10.md), not on compression.
