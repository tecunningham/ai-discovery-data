...
**Data:** [`../data/compression-records.csv`](../data/compression-records.csv), rows with series `hutter_enwik9` and `ltcb_enwik9`
**Upstream:** <http://prize.hutter1.net/> and <http://mattmahoney.net/dc/text.html>
**Verdict:** no acceleration

![Hutter Prize enwik9 records with a pending entry open and the uncapped leaderboard dashed.](../figures/discovery-algorithms-enwik9.png)
...
`compression_chart("hutter_enwik9", ...)` in
[`../tools/make_figures.py`](../tools/make_figures.py) reads
`compression-records.csv`, keeps the rows whose `series` column matches, and
...
[enwik8 series](algorithms-enwik8.md) in this collection is the same prize's own
