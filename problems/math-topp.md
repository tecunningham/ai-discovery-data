...
![Cumulative dated solved, settled, or closed statuses in The Open Problems Project.](../figures/discovery-math-topp.png)
...
on the [Millennium](math-millennium.md) problems. What it is not is a fixed-cohort
...
`problem_list_chart("topp", ...)`, called from `math_charts()` in
[`../tools/make_figures.py`](../tools/make_figures.py), filters
`famous-open-problem-lists.csv` on `list_id`, keeps rows whose `status` is
...
[Millennium](math-millennium.md) and [Hilbert](math-hilbert.md) zeros are. What
...
[Hilbert](math-hilbert.md), [Smale](math-smale.md) and
[Millennium](math-millennium.md); the corpus with measurable AI flow is
[Erdős](math-erdos.md).
