...
**Data:** [`../data/ossfuzz-discoveries.csv`](../data/ossfuzz-discoveries.csv)
...
![Annual OSS-Fuzz vulnerability records, falling from 1,041 in 2020 to 244 in 2025.](../figures/discovery-cyber-oss-fuzz.png)
...
draws a single amber bar per year from the `discoveries` column of
`ossfuzz-discoveries.csv`. Amber is the fuzzer colour throughout this
...
The CSV is built by `build_ossfuzz()` in
...
onward — 1,041 against 1,031 for 2020, 710 against 716 for 2022 — so the series
is reported from 2020 and the 267 earlier records are dropped.
...
for are [curl](cyber-curl.md), [OpenSSL](cyber-openssl.md) and
[Firefox](cyber-firefox.md).
