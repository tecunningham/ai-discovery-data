...
**Data:** [`../data/alphaevolve-records.csv`](../data/alphaevolve-records.csv), problems 6.44 and 6.3
...
![Record lower bounds on the sums-and-differences and autoconvolution constants, with AI and human steps distinguished.](../figures/discovery-math-sums-autoconvolution.png)
...
`alphaevolve_value_chart(["6.44", "6.3"], ...)`, called from `math_charts()` in
[`../tools/make_figures.py`](../tools/make_figures.py), filters
`alphaevolve-records.csv` to those two `problem` values with a non-empty `value`
...
same frame are [kissing number](math-kissing-11.md) and
[the finite-construction groups](math-alphaevolve-related-records.md); the
century-scale series with no AI step are
[analytic-number-theory exponents](math-antedb.md) and
[sphere packing](math-sphere-packing.md).
