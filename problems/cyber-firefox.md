...
**Data:** [`../data/firefox-advisories.csv`](../data/firefox-advisories.csv); per-reporter rows in [`../data/discovery-finders.csv`](../data/discovery-finders.csv)
...
![Annual Firefox vulnerability disclosures, split by explicit AI and fuzzer credit.](../figures/discovery-cyber-firefox.png)

## The problem

Mozilla publishes one YAML file per security advisory, and each CVE inside it
carries a `reporter` string. That makes Firefox the third fixed codebase in this
...
announcement year. It is a disclosure count, not a count of bugs found or bugs
remaining.
...
`cyber_stacked()`, called from `cyber_charts()` in
...
many CVEs, and the amber band can be compared with the red one by eye.

The CSV is built by `build_firefox()` in
[`../tools/fetch/collective_progress.py`](../tools/fetch/collective_progress.py),
...
string with two regexes. `FIREFOX_AI` matches Claude, Anthropic, OpenAI, GPT,
Gemini, Big Sleep, Mythos, the AI-security firms, and the bare words "LLM" and
"agent"; `FIREFOX_FUZZ` matches "fuzz". AI is tested first, so a report crediting
a model and a fuzzer counts as AI. Pre-2016 advisories do not list CVEs in this
structure, which is where the series starts.
...
AI-credited CVEs in 2026, 121 are credited to a single seven-person team —
...
tool. The remainder is 11 CVEs credited to Amy Burnett of OpenAI, 2 to Artur
...
[OSS-Fuzz](cyber-oss-fuzz.md), which runs the other way over the same period,
and the two smaller fixed codebases are [curl](cyber-curl.md) and
[OpenSSL](cyber-openssl.md). Every count here is an aggregation of Mozilla's
