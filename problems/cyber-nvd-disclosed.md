...
**Data:** [`../data/nvd-kev-by-year.csv`](../data/nvd-kev-by-year.csv), finer grain in [`../data/nvd-kev-by-quarter.csv`](../data/nvd-kev-by-quarter.csv)
...
![Annual CVE disclosures in the US National Vulnerability Database.](../figures/discovery-cyber-nvd-disclosed.png)
...
A "discovery" here is one CVE record, counted in the year NVD published it.
...
`cyber_simple_bars()`, called from `cyber_charts()` in
[`../tools/make_figures.py`](../tools/make_figures.py),
draws one bar per year from the `nvd_published` column of `nvd-kev-by-year.csv`.
...
The same CSV carries the `kev_added` column plotted in
[all software: known exploited](cyber-kev-exploited.md). The two are drawn as
...
The CSV is built by `build_nvd_and_kev()` in
...
script reads `totalResults` rather than the records themselves.
...
[all software: known exploited](cyber-kev-exploited.md), and the fixed-codebase
alternative with named finders is [curl](cyber-curl.md). For capability measured
